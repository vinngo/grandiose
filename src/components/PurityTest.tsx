...
const PurityTest = () => {
  ...
  const allQuestions = useMemo(
    () => prompts.flatMap((category) => category.items.map((item) => ({ id: item.id, text: item.text }))),
    []
  )
  ...
  const ScoreContent = () => (
    <div className="p-4 mb-6 text-center">
      ...
      <h1 className="text-4xl text-red-600 font-bold mb-4">{score}</h1>
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-4">
        ...
        <button
          ...
          className="px-4 py-1 hover:bg-gray-100 flex space-x-1 border border-black items-center"
        ...
        >
          ...
          >
           ...
           <span className="hidden sm:inline mr-1">Copy share link</span>
        </button>
        ...
        >
          <span className="hidden sm:inline mr-1">Share to </span>
          <svg
            xmlns="http://www.w3.org/2000/svg\"
            width="16"
            height="16"
  ...
  return score === null ? <FormContent /> : <ScoreContent />;
}
